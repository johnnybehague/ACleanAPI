...
public class AcCrudControllerBase_CreateEntityAsyncTests
{
    ...
    public async Task CreateEntityAsync_ModelStateInvalid_ReturnsBadRequestWithModelState()
    {
        ...
        var requestMock = new Mock<IAcCreateEntityRequest<UserTestDto>>();

        // Act
        var result = await _controller.CreateEntityAsync(requestMock.Object);

        // Assert
        var badRequest = result as BadRequestObjectResult;
        Assert.IsNotNull(badRequest);
        Assert.IsFalse(_controller.ModelState.IsValid);
        _mediatorMock.Verify(m => m.Send(It.IsAny<IAcCreateEntityRequest<UserTestDto>>(), It.IsAny<CancellationToken>()), Times.Never);
    }
